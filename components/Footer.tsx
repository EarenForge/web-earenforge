  ...
  return (
    <footer className="border-t border-border bg-background transition-colors duration-300">
      <div className="max-w-5xl mx-auto px-6 py-12">
        <div className="grid md:grid-cols-3 gap-8 mb-8">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <Image src="/logo.png" alt="Earen Forge" width={32} height={32} />
              <span className="font-semibold">Earen Forge</span>
            </div>
            <p className="text-muted text-sm">{t.footer.description}</p>
          </div>
            ...
            <div className="flex gap-3">
              {socials.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  className="text-muted hover:text-foreground hover:bg-card p-2 rounded-lg transition"
                >
                  {s.icon}
                </a>
              ))}
        ...
        <div className="border-t border-border pt-6 text-center text-muted-strong text-sm">
          <p>&copy; {new Date().getFullYear()} Earen Forge. {t.footer.copyright}</p>
        </div>
      </div>
    </footer>
  );
